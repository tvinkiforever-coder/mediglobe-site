...
var MG_INS = { ekta: '', safetywing: 'https://safetywing.com/?referenceID=26566970&utm_source=26566970&utm_medium=Ambassador', cis: ['RU', 'UA', 'BY', 'AE'] };
class App {
  state = { tileW: 116 };
  _tiles = 12;
  componentDidMount() {
  ...
  _initInsurance() {
    var cta = document.getElementById('mgInsCta');
    if (!cta) return;
    fetch('/api/geo').then(function (r) { return r.json(); }).then(function (d) {
      var cc = (d && d.country) || '';
      if (MG_INS.cis.indexOf(cc) >= 0 && MG_INS.ekta) { cta.setAttribute('href', MG_INS.ekta); }
    }).catch(function () {});
  }
  _initScrollCards() {
    if (this._sc) return;
